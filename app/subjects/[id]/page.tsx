...
function subjectColor(name: string): ColorKey {
  const colors: ColorKey[] = ["indigo", "violet", "emerald"];
  let hash = 0;
...
export default function SubjectPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setLoadError(null);
      try {
        const [subjects, mats] = await Promise.all([
          apiFetch<Subject[]>("/api/subjects"),
          apiFetch<Material[]>(
            `/api/materials?subject_id=${encodeURIComponent(subjectId)}`,
          ),
        ]);
  ...
  const styles = colorStyles[color];
  const uploadHref = `/upload?subject_id=${encodeURIComponent(subjectId)}`;
        ...
        {materials.length === 0 ? (
          <div className="bg-card rounded-xl border border-border border-dashed p-12 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-muted-foreground mb-4 max-w-sm">
              Upload PDF study materials to get started with AI-powered learning.
            </p>
            <Link href={uploadHref}>
              <Button className="gap-2">
